...
def writeJsonToFile(json_dump, globPattern, fileType):
    if 'master' in globPattern:
        fileName = 'master-coverage-report.json'
    else:
        fileName = 'pr-coverage-report.json'

    old_json = {}
    # check file already exists, store old json data
    # this is usefull in case of storing combined coverage data for cobertura and jest-junit report
...
def parseCobertura(files):
    required_attributes = ['lines-valid', 'line-rate', 'lines-covered']
    coverage_metrics = {}
...
def main(fileName, fileType):
    ...
    json_dump = getJsonFromXml(fileName, fileType)
    writeJsonToFile(json_dump, fileName, fileType)
